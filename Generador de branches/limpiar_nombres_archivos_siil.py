...
def creacion_branches():
    ...
    for cliente in range(len(clientes)):
        ...
        archivos = f'cp -Rv {ruta_del_servidor_producto_clientes}{rutas_clientes[cliente]} {ruta_repositorio}/Personalizaciones'
        os.system(archivos)
        repo.git.add(A=True)
        git.commit('-m', f'{azul_agua}Creacion de branch para cliente: {endcolor}' + clientes[cliente])
        print(' '+verde)
